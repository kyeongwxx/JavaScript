function* genFunc() {
    ...
    // 이 때 yield된 값 1은 next 메소드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에 할당된다.
    ...
    // 이 때 yield된 값 x + 10은 next 메소드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에 할당된다.
    const y = yield (x + 10);

    // 세 번째 next 메소드를 호출할 때 전달한 인수 20은 두 번째 yield 표현식을 할당받는 y 변수에 할당된다.
    // 즉, const y = yield (x + 10);는 세 번째 next 메소드를 호출했을 때 완료된다.
    // 세 번째 next 메소드를 호출하면 함수 끝까지 실행된다.
    // 이 때 제너레이터 함수의 반환값 x + y는 next 메소드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에 할당된다.
...
// next 메소드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에는 첫 번째 yield된 값 1이 할당된다.
let res = generator.next();
console.log(res);

// next 메소드에 인수로 전달한 10은 getFunc 함수의 x 변수에 할당된다.
// next 메소드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에는 두 번째 yield된 값 20이 할당된다.
res = generator.next(10);
console.log(res);

// next 메소드에 인수로 전달한 20은 getFunc 함수의 y 변수에 할당된다.
// next 메소드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에는 두 번째 yield된 값 30이 할당된다.
